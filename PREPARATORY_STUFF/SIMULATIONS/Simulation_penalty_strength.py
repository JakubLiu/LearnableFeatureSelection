...
penalty_strengths = list(range(0, 20, 5))
cols = ['penalty_strength', 'mean_correct_inclusion_rate', 'mean_correct_exclusion_rate', 'mean_summary_score']
simulation_table = pd.DataFrame(index=range(len(penalty_strengths)), columns=cols)


for i in range(0, len(penalty_strengths)):

    print('===============================threshold: {}========================================='.format(penalty_strengths[i]), flush=True)
    simulation_table.iloc[i,0] = penalty_strengths[i]

    sim = wrp.Wrapper(thresh=0.5,
        penalty_strength=penalty_strengths[i],
        optimizer='AdamW',
        n_epochs=1000,
        n_rep=100,
        X_train=X_train,
...
np.savetxt('/media/DANE/home/jliu/LEARNABLE_FS/PREPARATORY_STUFF/SIMULATIONS/Simulation_penalty_strength.txt', simulation_table)
